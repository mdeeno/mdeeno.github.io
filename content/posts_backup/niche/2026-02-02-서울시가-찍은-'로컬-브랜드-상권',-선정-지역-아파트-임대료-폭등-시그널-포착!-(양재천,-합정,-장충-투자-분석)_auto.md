...
tags:
  [
    '서울 로컬 브랜드 상권, 양재천 임대료, 합정 부동산 투자, 상권 육성 사업',
    '부동산투자',
    '재테크',
  ]
...
<p style="font-size:10px;color:#ccc;margin-bottom:10px;text-align:right;">이 포스팅은 애드릭스 수익을 위해 작성되었습니다.</p><img src="https://raw.githubusercontent.com/mdeeno/mdeeno.github.io/main/static/images/chart-1770119110.png" alt="전망 차트" style="width:100%; max-width:800px; height:auto; display:block; margin:20px auto; border-radius:8px; box-shadow:0 4px 15px rgba(0,0,0,0.1);">
...
<script>
  async function submitBlogLead() {
    ...
    const { error } = await sb.from('lead_emails').insert([{ 
        email: email, 
        source: 'blog: 서울시가 찍은 '  // 🔥 이 부분이 실제 제목으로 바뀝니다!
    }]);
