...
draft: false
categories: ["자산 전략"]
tags: ["HUG 공매, 전세 사기 피해 주택, 우선 매수권", "부동산투자", "재테크"]
...
<script>
  async function submitBlogLead() {
    ...
    const { error } = await sb.from('lead_emails').insert([{ 
        email: email, 
        source: 'blog: 전세 사기 집, 내가 직접 산다! HUG 공매 물건 셀프 낙찰 꿀팁과 우선 매수권 활용법'  // 🔥 이 부분이 실제 제목으로 바뀝니다!
    }]);
