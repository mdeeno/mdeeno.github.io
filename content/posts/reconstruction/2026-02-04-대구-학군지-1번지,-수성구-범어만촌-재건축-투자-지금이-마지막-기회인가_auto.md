...
draft: false
categories: ["정비 사업"]
tags: ["대구 수성구 재건축, 범어동 재건축, 만촌동 학군지", "부동산투자", "재테크"]
...
<p style="font-size:10px;color:#ccc;margin-bottom:10px;text-align:right;">이 포스팅은 애드릭스 수익을 위해 작성되었습니다.</p><img src="https://raw.githubusercontent.com/mdeeno/mdeeno.github.io/main/static/images/chart-1770303133.png" alt="전망 차트" style="width:100%; max-width:800px; height:auto; display:block; margin:20px auto; border-radius:8px; box-shadow:0 4px 15px rgba(0,0,0,0.1);">
...
<script>
  async function submitBlogLead() {
    ...
    const { error } = await sb.from('lead_emails').insert([{ 
        email: email, 
        source: 'blog: 대구 학군지 1번지, 수성구 범어/만촌 재건축 투자: 지금이 마지막 기회인가?'  // 🔥 이 부분이 실제 제목으로 바뀝니다!
    }]);
