---
title: '부동산 & 재건축 필수 계산기 모음'
summary: '재건축 이주비, DSR, 취득세, 양도세, 이자 — 자금 계획에 필요한 모든 계산기를 한 곳에서'
hidemeta: true
---

<div style="background-color:#fff5f5;border:2px solid #ff6b6b;border-radius:10px;padding:20px;margin-bottom:30px;box-shadow:0 4px 6px rgba(0,0,0,0.05);">
  <strong style="font-size:16px;color:#c92a2a;">사용 전 필독 사항</strong>
  <p style="margin:8px 0 0;font-size:14px;line-height:1.6;color:#495057;">
    본 계산기는 최신 법령 및 금리 기준으로 제작되었으나, 개인 상황이나 은행 정책에 따라 실제와 다를 수 있습니다.
    이자율·세율은 반드시 직접 확인 후 입력하세요.
  </p>
</div>

## 재건축 조합원 필수 계산기

재건축 투자에서 자금 계획은 가장 중요한 변수입니다.
이주비 한도, 이자 부담, 취득세, 양도세를 사전에 시뮬레이션하면 분담금 리스크를 크게 낮출 수 있습니다.

- **[DSR & 대출 한도 계산기](/calculators/calc_dsr/)** — 이주비·중도금 대출 가능 여부 미리 확인
- **[대출 이자 계산기](/calculators/calc_interest/)** — 이주비 후불이자 총액 시뮬레이션
- **[취득세 계산기](/calculators/calc_tax/)** — 조합원 아파트 매수 시 취득 비용 계산
- **[양도소득세 계산기](/calculators/calc_transfer/)** — 현금청산·매도 시 세금 시뮬레이션
- **[보유세(재산세+종부세) 계산기](/calculators/calc_hold/)** — 재건축 대기 기간 보유 비용 계산

<div style="margin:24px 0;padding:20px 28px;background:linear-gradient(135deg,#eff6ff,#dbeafe);border:2px solid #2563eb;border-radius:12px;text-align:center;">
  <p style="font-size:1.05rem;font-weight:700;color:#1e40af;margin:0 0 8px;">계산기로 한계를 느끼셨나요?</p>
  <p style="font-size:0.9rem;color:#374151;margin:0 0 14px;">M-DEENO 정밀 리포트는 내 단지의 공사비·비례율·분담금을 종합 시뮬레이션합니다.</p>
  <a href="https://mdeeno.com/member" target="_blank" rel="noopener noreferrer"
     style="display:inline-block;padding:12px 28px;background:#2563eb;color:#fff;font-weight:700;border-radius:8px;text-decoration:none;">
    M-DEENO 정밀 분석 받기 →
  </a>
</div>

---

## 매수/매도 (세금 & 비용)

- **[중개보수(복비) 계산기](/calculators/calc_fee/)**
- **[취득세 계산기](/calculators/calc_tax/)**
- **[양도소득세 계산기](/calculators/calc_transfer/)**

## 대출 & 자금 계획

- **[DSR & 대출 한도 계산기](/calculators/calc_dsr/)**
- **[대출 이자 계산기](/calculators/calc_interest/)**

## 보유 & 청약

- **[보유세(재산세+종부세) 계산기](/calculators/calc_hold/)**
- **[청약 가점 계산기](/calculators/calc_subscription/)**
- **[전월세 전환율 계산기](/calculators/calc_rent/)**

## 소득 관리

- **[연봉 실수령액 계산기](/calculators/calc_salary/)**
